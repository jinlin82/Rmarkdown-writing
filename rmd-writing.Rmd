...
knitr::include_graphics(".\\results\\知网.png")
```
...
knitr::include_graphics(".\\results\\语言设置.png")
```
...
进行搜索，搜索成功以后点击图 \@ref(fig:fig10) 中红色框内容即可下载参考bibtex。）
...
```{r fig10,eval=T, echo=F,fig.cap="英文文献搜索",dev="png",results='markup', cache=F}
knitr::include_graphics(".\\results\\英文文献搜索.png")
```

6. 第3步中设置的bibtexkey此时还未起作用，需要手动更新：点击其中一条文献，然后按crtl+A全选，点击tool-autogen
erate bib tex key,或者直接点击图 \@ref(fig:fig11) 菜单栏中的"钥匙"，则全部bibtexkey都得到更新。

```{r fig11,eval=T, echo=F,fig.cap="钥匙",dev="png",results='markup', cache=F}
...
name输入JAVA_HOME，点击ok，返回user variables窗口，输入jre所在的路径。
...
的路径（注：修改之前需要下载gs）,如图\@ref(fig:fig12) 所示。

```{r fig12,eval=T, echo=F,fig.cap="Equalx安装",dev="png",results='markup', cache=F}
...
选择“LaTex 2.09 and lator”，如图 \@ref(fig:fig13) 所示，设置完成以后点击convert就能将word
文档中的公式转化为LaTex公式。
注：若安装不成功，卸掉重新安装mathtype即可。

```{r fig13,eval=T, echo=F,fig.cap="Mathtype安装",dev="png",results='markup', cache=F}
